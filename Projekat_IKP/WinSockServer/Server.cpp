...
std::map<int, int> workerToClientMap;
int clientCounter = 0;
...
// Distribucija podataka pri prijavljivanju novog Worker Role-a
void DistributeDataOnRegistration(int newWorkerRoleID) {
	// Provera da li postoji bar jedan Worker Role
	if (numOfWorkerRoles > 0) {
		// Računanje ukupnog broja podataka u redu
		int totalFiles = queue->size;

		// Računanje broja podataka koje će svaki Worker Role dobiti (podela ravnomerno)
		int filesPerWorker = totalFiles / numOfWorkerRoles;

		// Dodatno, računanje koliko podataka će dobiti prvih (totalFiles % numOfWorkerRoles) Worker Role-ova
		int additionalFiles = totalFiles % numOfWorkerRoles;

	/*for (int i = 0; i < numOfWorkerRoles; i++) {
			

			// Ako je trenutni Worker Role među onima koji dobijaju dodatne podatke, dodaj mu 1
			if (i < additionalFiles) {
				workerRolesInfo[i].storedFiles++;
			}
		}*/
	}
}

// Ažuriranje Load Balancer-a
void UpdateLoadBalancer() {
	// Implementirajte ažuriranje Load Balancer-a prema novim informacijama
	printf("Load Balancer updated. Number of Worker Roles: %d\n", numOfWorkerRoles);
}
// Funkcija za prijavljivanje Worker Role-a
void RegisterWorkerRole(int workerRoleID) {
	// Dodaj Worker Role u niz
	//workerRolesInfo[numOfWorkerRoles].workerRoleID = workerRoleID;
	//workerRolesInfo[numOfWorkerRoles].storedFiles = 0;

	// Povećaj broj Worker Role-ova
	numOfWorkerRoles++;

	// Distribuiraj podatke na sve Worker Role-ove
	DistributeDataOnRegistration(workerRoleID);

	// Ažuriraj Load Balancer
	UpdateLoadBalancer();
}
...
DWORD WINAPI workerRole(LPVOID lpParam) {
	...
	while (WaitForMultipleObjects(semaphore_num, semaphores, FALSE, INFINITE) == WAIT_OBJECT_0 + 1) {
		...
		printf("Hello from workerRole %d. Remove data: %d\n", n, workerRoleData);
